...
로컬에서는 applicant Vite 앱을 `http://localhost:5173`, operator Vite 앱을 `http://localhost:5174`에서 실행합니다. 각 Vite 서버는 `/api`를 기본 `http://localhost:8080` 백엔드로 프록시하면서 신뢰된 `X-HSU-Frontend` 값을 붙입니다. 백엔드 주소가 다르면 프론트 실행 전에 `HSU_BACKEND_ORIGIN`을 설정합니다. 위 두 콜백 URI를 Kakao Developers에도 등록해야 합니다.
