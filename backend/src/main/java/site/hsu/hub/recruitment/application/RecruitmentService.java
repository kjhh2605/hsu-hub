...
public class RecruitmentService implements RecruitmentApplicationReader {
    ...
    public RecruitmentService(RecruitmentRepository r, RecruitmentStageRepository s, ApplicationFormRepository f,
                              ...
                              ClubScope c, CurrentUser current, ObjectMapper mapper) {
        recruitments=r; stages=s; forms=f; steps=st; questions=q; options=o;
        clubScope=c; this.current=current; this.mapper=mapper;
    }

    @Transactional
    public RecruitmentView publish(Long clubId, PublishCommand cmd) {
        new RecruitmentPeriod(cmd.opensAt(), cmd.closesAt());
        if (cmd.title()==null || cmd.title().isBlank() || cmd.title().length()>160 || cmd.quota()<1)
        ...
        var entity = recruitments.save(new RecruitmentEntity(clubId, cmd.title(), cmd.quota(), cmd.opensAt(),
                cmd.closesAt(), json(cmd.contentBlocks()), current.id()));
        int i=0;
    ...
    public Snapshot requireOpen(Long id,Instant now) {
        var r=find(id);
        if(new RecruitmentPeriod(r.opensAt(),r.closesAt()).stateAt(now)!=RecruitmentState.OPEN)
            throw new ApiException(ErrorCode.CONFLICT,"현재 지원 가능한 모집이 아닙니다.");
        return new Snapshot(r.id(),r.clubId(),r.title(),r.opensAt(),r.closesAt());
    }
    ...
    private RecruitmentView view(RecruitmentEntity r){return new RecruitmentView(r.id(),r.clubId(),r.title(),r.quota(),r.opensAt(),r.closesAt(),new RecruitmentPeriod(r.opensAt(),r.closesAt()).stateAt(Instant.now()).name(),readJson(r.contentBlocks()),stages.findByRecruitmentIdOrderByOrder(r.id()).stream().map(s->new StageView(s.type(),s.label(),s.startsAt(),s.endsAt(),s.enabled())).toList(),r.publishedAt());}
    private String json(JsonNode n){try{return mapper.writeValueAsString(n==null?mapper.createArrayNode():n);}catch(JsonProcessingException e){throw new ApiException(ErrorCode.BAD_REQUEST);}}
    private JsonNode readJson(String s){try{return mapper.readTree(s);}catch(JsonProcessingException e){throw new IllegalStateException(e);}}
    private static void validateStages(List<StageCommand> stages){if(stages==null)throw new ApiException(ErrorCode.VALIDATION_FAILED);Set<String> types=Set.of("DOCUMENT","DOCUMENT_RESULT","INTERVIEW","FINAL_RESULT");for(var x:stages)if(!types.contains(x.type())||x.label()==null||x.label().isBlank()||(x.enabled()&&(x.startsAt()==null||x.endsAt()==null||!x.startsAt().isBefore(x.endsAt()))))throw new ApiException(ErrorCode.VALIDATION_FAILED,"전형 일정을 확인해 주세요.");}
    public record RecruitmentView(Long id,Long clubId,String title,int quota,Instant opensAt,Instant closesAt,String state,JsonNode contentBlocks,List<StageView> stages,Instant publishedAt){}
